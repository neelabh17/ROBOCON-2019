...
void robocon()
{
	while (!PS4.connected())
	{
		Serial.print("PS$ not connected yet");
	}
	while (!PS4.getButtonClick(UP))
	{
		Serial.print("We have not got the required input that is the up key");
	}
...
void isKhagaiReqAgain()
{
	...
	if (PS4.getButtonClick(UP))
	{
		Serial.print("We are shifting to manual");
		khagai();
		return;
	}
}
...
void phase_1(long refDatum)
{
	...
	while (e < 6000)
	{
		...
		e = abs(myEnc.read());
		e = -refDatum;
		Serial.println("P1 diag");
		isSplManualReq();
		return;
	}

	rest(150);
	while (abs(myEnc.read()) - refDatum < 15000)
	{
		isSplManualReq();
		return;
		sensors_event_t event;
...
void phase_2(long refDatum)
{
	...
	while (abs(myEnc.read()) - refDatum < 34300)
	{
		isSplManualReq();
		return;
		sensors_event_t event;
...
void phase_3(long refDatum)
{
	...
	while (abs(myEnc.read()) - refDatum < 53000)
	{
		isSplManualReq();
		return;
		sensors_event_t event;
...
void phase_5(long refDatum)
{
	...
	while (abs(myEnc.read() - refDatum) <= 98800)
	{
		isSplManualReq();
		return;
		sensors_event_t event;
...
void internalsOfManual()
{
	...
	{
		int ps3y = PS4.getAnalogHat(LeftHatX) - 127;
	...
	v = add_vels(p4, p2); // straight + rotate
	write_motor_speeds(v);
	if (PS4.getAnalogHat(LeftHatX) < 137 && PS4.getAnalogHat(LeftHatX) > 117 && PS4.getAnalogHat(LeftHatY) < 137 && PS4.getAnalogHat(LeftHatY) > 117 && (psw < 20 && psw > -20))
	{
		write_motor_speeds(move_angr(0, 0));
	}
	...
	dif = x - setpoint;
	Usb.Task();
	if (PS4.getButtonClick(TRIANGLE) && triangle_check == true)
	{
		PS4.setLed(Yellow);
		phase_5(datum);
		phase_6(datum);
